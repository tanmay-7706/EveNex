...
export default function RegisterModal({ event, isOpen, onClose }) {
  ...
  const { mutate: sendTicketEmail } = useConvexMutation(api.actions.sendTicketEmail);
    ...
    try {
      ...
      try {
        await sendTicketEmail({
          ...
          attendeeName: name,
          eventDate: format(event.startDate, "PPP 'at' p"),
          eventLocation: event.locationType === "online" 
            ? "Online Event" 
            : `${event.city}, ${event.state || event.country}`,
        });
      } catch (emailError) {
        console.error("Email failed:", emailError);
        toast.warning("Registration successful, but confirmation email failed to send");
      }
